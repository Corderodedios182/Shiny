---
title: "Mitsubishi México Febrero 2019 - 04 Marzo 2019"
output: 
...
url = "https://taxonomybuilder.oneomg.com/Client/116"
download.file(url, destfile = "scrapedpage.html", quiet=TRUE)
url_html <- read_html("scrapedpage.html")


ruta <- paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/SOCIAL MEDIA/",mes,sep = '')
setwd(paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/SOCIAL MEDIA/",mes, sep = ''))



gs_auth(new_user = TRUE) # Iniciar sesión
base <- gs_read(gs_title('Social Media Mitsubishi'), ws = gs_ws_ls(ss = gs_title("Social Media Mitsubishi"))[grep(mes,gs_ws_ls(ss = gs_title("Social Media Mitsubishi")))])
...
fuentes_sm <- fuentes_sm[c(grep('ad name',fuentes_sm[,1]),grep('anuncios',fuentes_sm[,1]),grep('campaña',fuentes_sm[,1])),]
...
#relaciones <- gs_read(gs_title("UNIVERSO Y AUDICIA PLANTILLA MITSUBISHI"), ws = gs_ws_ls(ss = gs_title("UNIVERSO Y AUDICIA PLANTILLA MITSUBISHI"))[1])
...
for (i in 3:(length(str_split(estructura[2,3], '_')[[1]])+1)) {
...
colnames(fecha_anuncios) <- c('Dia','Sep1','Mes','Sep2','Año','Longitud','Val_dia','Val_sep1','Val_mes','Val_sep2','Val_año','Total_score','Evaluación',
...
for (i in seq(dim(fecha_anuncios)[1])) {
  if(fecha_anuncios[i,20] == 10 && fecha_anuncios[i,26] == 5){
    fecha_anuncios[i,28] <- 'BIEN'
  }else if(fecha_anuncios[i,20] != 10 || fecha_anuncios[i,26] != 5){
    fecha_anuncios[i,28] <- 'MAL'
  }
}


eval_contenido_anuncios[,c( "Fecha de fin")] <- fecha_anuncios[,c(28)]
...
#    eval_final[i,1] <- "CORRECTA ESTRUCTURA Y CATALOGO"
...
save(sm_hist_adname,file = paste(getwd(),'/Mitsubishi_Social_Media_Conjunto_de_Anuncios_',mes,'.rda', sep = ''))
...
save(sm_hist_anuncios,file = paste(getwd(),'/Mitsubishi_Social_Media_Anuncios_',mes,'.rda', sep = ''))
...
save(sm_hist_campania,file = paste(getwd(),'/Mitsubishi_Social_Media_Campaña_',mes,'.rda',sep = ''))
...
drive_rm(paste("~/Mitsubishi/Social Media/",mes,sep = ''))

drive_mkdir(paste("~/Mitsubishi/Social Media/",mes,sep = ''))

write.csv(resumen, file = paste(ruta,'/Mitsubishi_Social_Media_Resumen.csv',sep = ''), row.names = F)

write.csv(adname_sm, file =  paste(ruta,'/Mitsubishi_Social_Media_Anuncios.csv',sep = ''), row.names = F)

drive_upload(paste(ruta,'/Mitsubishi_Social_Media_Anuncios.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/Social Media/',mes,sep = '')), type = 'spreadsheet')

write.csv(anuncios_sm, file = paste(ruta,'/Mitsubishi_Social_Media_Conjunto_de_Anuncios.csv',sep = ''), row.names = F)

drive_upload(paste('Mitsubishi_Social_Media_Conjunto_de_Anuncios.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/Social Media/',mes,sep = '')), type = 'spreadsheet')

write.csv(campania_sm, file = paste(ruta,'/Mitsubishi_Social_Media_Campania.csv',sep = ''), row.names = F)

drive_upload(paste('Mitsubishi_Social_Media_Campania.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/Social Media/',mes,sep = '')), type = 'spreadsheet')
...
ruta <- paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/DCM/",mes,sep = '')

setwd(paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/DCM/",mes, sep = ''))
...
base_dcm <- gs_read(gs_title('DCM Mitsubishi'), ws = gs_ws_ls(ss = gs_title("DCM Mitsubishi"))[grep(mes,gs_ws_ls(ss = gs_title("DCM Mitsubishi")))])
...
campania_dcm[is.na(campania_dcm)] <- ''


write.csv(campania_dcm, file = 'Mitsubishi_DCM_Campania.csv', row.names = F)
...
columnas <- grep(FALSE,is.element(colnames(creativo),c('Fecha de inicio',' Fecha de fin','Fecha de implementación')))[2:length(grep(FALSE,is.element(colnames(creativo),c('Fecha de inicio',' Fecha de fin','Fecha de implementación'))))]
...
fecha[,1] <- substring(creativo[,"Fecha de implementación"],1,2)
fecha[,2] <- substring(creativo[,"Fecha de implementación"],3,3)
fecha[,3] <- substring(creativo[,"Fecha de implementación"],4,5)
fecha[,4] <- substring(creativo[,"Fecha de implementación"],6,6)
fecha[,5] <- substring(creativo[,"Fecha de implementación"],7,10)
fecha[,6] <- nchar(as.character(creativo[,"Fecha de implementación"]))
fecha[,7] <- as.numeric(is.element(fecha[,1],catalogo_fecha[,1]))
fecha[,8] <- as.numeric(is.element(fecha[,2],'-'))
fecha[,9] <- as.numeric(is.element(fecha[,3],as.character(na.omit(catalogo_fecha[,2]))))
fecha[,10] <- as.numeric(is.element(fecha[,4],'-'))
fecha[,11] <- as.numeric(is.element(fecha[,5],as.character(na.omit(catalogo_fecha[,3]))))
fecha[,12] <- apply(fecha[,7:11],1,sum)
fecha <- cbind(creativo[,"Fecha de implementación"],fecha)
colnames(fecha) <- c('Fecha','Dia','Sep1','Mes','Sep2','Año','Longitud','Val_dia','Val_sep1','Val_mes','Val_sep2','Val_año','Total_score','Evaluación')
...
eval_contenido_creativo_dcm[,(length(str_split(estructura_dcm[2,3], '_')[[1]]))] <- fecha[,14]
...
}else if(max(columnas_creativo_dcm[,2])== length(str_split(estructura_dcm[2,3], '_')[[1]]) && min(columnas_creativo_dcm[,2])< length(str_split(estructura_dcm[2,3], '_')[[1]]) ||   max(columnas_creativo_dcm[,2])== length(str_split(estructura_dcm[2,3], '_')[[1]]) && min(columnas_creativo_dcm[,2])< length(str_split(estructura_dcm[2,3], '_')[[1]]) || max(columnas_creativo_dcm[,2])== length(str_split(estructura_dcm[2,3], '_')[[1]]) && min(columnas_creativo_dcm[,2]) ==  length(str_split(estructura_dcm[2,3], '_')[[1]])){
...
creativo[is.na(creativo)] <-''

colnames(creativo)[1] <- c("Creative")

write.csv(creativo, file = paste(ruta,'/Mitsubishi_DCM_Creative.csv', sep = ''), row.names = F)
...
placement[is.na(placement)] <- ''

colnames(placement) <- c("Placement",colnames(placement)[2:(dim(placement)[2]-1)],'Evaluación')

write.csv(placement, file = paste(ruta,'/Mitsubishi_DCM_Placement.csv', sep = ''),  row.names = F)
...
write.csv(adname, file = paste(ruta,'/Mitsubishi_DCM_Adname.csv', sep = ''), row.names = F)


drive_rm(paste("~/Mitsubishi/DCM/",mes,sep = ''))

drive_mkdir(paste("~/Mitsubishi/DCM/",mes,sep = ''))

drive_upload(paste(ruta,'/Mitsubishi_DCM_Campania.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/DCM/',mes,sep = '')), type = 'spreadsheet')

drive_upload(paste(ruta,'/Mitsubishi_DCM_Creative.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/DCM/',mes,sep = '')), type = 'spreadsheet')

drive_upload(paste(ruta,'/Mitsubishi_DCM_Placement.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/DCM/',mes,sep = '')), type = 'spreadsheet')

drive_upload(paste(ruta,'/Mitsubishi_DCM_Adname.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/DCM/',mes,sep = '')), type = 'spreadsheet')
...
ruta <- paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/ADWORDS/",mes,sep = '')

setwd(paste("C:/Users/Luis.TorresP/Desktop/Archivos/Validador de Taxonomía/MITSUBISHI/ADWORDS/",mes, sep = ''))
base_adw <- gs_read(gs_title('ADW Mitsubishi'), ws = gs_ws_ls(ss = gs_title("ADW Mitsubishi"))[grep(mes,gs_ws_ls(ss = gs_title("ADW Mitsubishi")))])
...
}else if(max(columnas_cam_adw[,2])== length(str_split(estructura_adw[2,3], '_')[[1]]) && min(columnas_cam_adw[,2])< length(str_split(estructura_adw[2,3], '_')[[1]]) ||   max(columnas_cam_adw[,2])== length(str_split(estructura_adw[2,3], '_')[[1]])-1 && min(columnas_cam_adw[,2])< length(str_split(estructura_adw[2,3], '_')[[1]])-1 || max(columnas_cam_adw[,2])== length(str_split(estructura_adw[2,3], '_')[[1]])-1 && min(columnas_cam_adw[,2])== length(str_split(estructura_adw[2,3], '_')[[1]])-1 || max(columnas_cam_adw[,2])== length(str_split(estructura_adw[2,3], '_')[[1]]) && min(columnas_cam_adw[,2]) == length(str_split(estructura_adw[2,3], '_')[[1]])){
...
campania_adw[is.na(campania_adw)] <- ''

write.csv(campania_adw, file = paste(ruta,'/Mitsubishi_ADW_Campania.csv', sep = ''), row.names = F)

drive_rm(paste("~/Mitsubishi/ADW/",mes,sep = ''))

drive_mkdir(paste("~/Mitsubishi/ADW/",mes,sep = ''))

drive_upload(paste(ruta,'/Mitsubishi_ADW_Campania.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/ADW/',mes,sep = '')), type = 'spreadsheet')
...
}else if(max(columnas_cuenta_adw[,2]) > length(str_split(estructura_adw[3,3], '_')[[1]]) && min(columnas_cuenta_adw[,2]) < (length(str_split(estructura_adw[3,3], '_')[[1]])-1)){
...
write.csv(cuenta, file = paste(ruta,'/Mitsubishi_ADW_Cuenta.csv', sep = ''), row.names = F)


drive_upload(paste(ruta,'/Mitsubishi_ADW_Cuenta.csv',sep = ''),path =  as_dribble(paste('~/Mitsubishi/ADW/',mes,sep = '')), type = 'spreadsheet')
...
```{r,results='asis'}
tsm_1 <- kable(toupper(estructura[1,3]),format = 'html',longtable = TRUE, caption = paste('Estructura:',estructura[1,3]),
                     booktabs = TRUE)%>%kable_styling(latex_options = c("striped", "hold_position"),full_width = T)%>%row_spec(0, bold = T, color = "white", background = "red")
tsm_1
```
